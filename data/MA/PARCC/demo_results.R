...
sum_demo%>%mutate(demo = reorder(demo, pass_per))%>%ggplot(aes(x=demo, y=pass_per))+geom_col()+coord_flip()
...
sum_math%>%mutate(demo = reorder(demo, pass_per))%>%ggplot(aes(x=demo, y=pass_per))+geom_col()+coord_flip()
