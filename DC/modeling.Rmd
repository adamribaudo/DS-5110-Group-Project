...
# Modeling
```{r}
dc_part <- resample_partition(dc, c(train = 0.6,test = 0.4))
dc_train <- dc_part$train$data
dc_test <- dc_part$test$data
```
...
>>>>>>> d3015683bbb5536b7db80bc0eb7e44d1137ce146
...
grid <- dc %>% 
  data_grid(suspensions, Safe_Truancy_Current_Year) %>% add_predictions(fit3) %>% na.omit() %>% 
...
ggplot(grid) + geom_point(aes(x=suspensions, y=Safe_Truancy_Current_Year, color=pred_binary)) +
  ...
  labs(x="Suspension Rate", y="Truancy Rate", color="Above Avg Math", title="Predicted Inputs to Achieve Above Average Math Proficiency") + theme_light()
