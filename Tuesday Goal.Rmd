...
scores%>%
  ...
  mutate(Truancy_Percent = Truancy_Rate * 100) %>%
  ggplot(aes(x = Truancy_Percent,
                      y = as.numeric(Percent_Met_or_Exceeded_Expectations)),na.rm = TRUE) + geom_point() +
  geom_smooth()+  
  scale_y_continuous() +
    xlab("Truancy Rate") +
    ylab("% Met or Exceeded Expectations")+
  scale_x_continuous()
...
turnover <- turnover%>%filter(Job_Classification == 'Teachers',!is.na(Turnover_Rate))%>%
            transmute(Retention_Rate = as.numeric(1-Turnover_Rate),District_Name=District_Name)

scores%>%
  left_join(turnover, by = 'District_Name') %>%
  mutate(r2 = Retention_Rate * Retention_Rate * 100) %>%
  ggplot(aes(x = r2,
...
final_data <- scores%>%
  ...
  left_join(turnover, by = 'District_Name') %>%
  mutate(r2 = Retention_Rate * Retention_Rate * 100)%>%
  filter(!is.na(Percent_Met_or_Exceeded_Expectations))

fitdata<-lm(as.numeric(Percent_Met_or_Exceeded_Expectations) ~ Truancy_Percent + Suspensions + r2,data = final_data)
summary(fitdata)
