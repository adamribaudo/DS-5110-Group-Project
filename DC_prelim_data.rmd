...
```{r}
  ggplot(dc) + geom_boxplot(aes(x = Uniform, y=`School Code`  (math_proficient/math_tests)))
```

# Exploring Truancy
- Defined as unexcused absence from school
...
- TODO show median truancy by proficiency

```{r}

dc %>% group_by(Dir_Ward) %>% summarise(med_truancy = median(Safe_Truancy_Current_Year, na.rm=T), proficiency = proficient/total)
